...
#							next if $Common::ORIGAMIBetas [$b] == 0.8;
							...
							foreach $data_base (@Common::DataBases)
							{
								...
								make_test_classifier_or ($data_base, $Common::ClassifierSupports[$s], $Common::Confidences[$c], $Common::MinNumRules[$n], $Common::MaxNumRankRules[$l], $Common::ClassifierOMetrics[$e], $Common::ORIGAMIAlphas[$a], $Common::ORIGAMIBetas[$b], \$tim, \$pat, \$rul, \$acc);
							...
							save_output_file ('average', $Common::ClassifierSupports[$s], $Common::Confidences[$c], $Common::MinNumRules[$n], $Common::MaxNumRankRules[$l], $Common::ClassifierOMetrics[$e], $Common::ORIGAMIAlphas[$a], $Common::ORIGAMIBetas[$b], $time_avg, $patterns_avg, $rules_avg, $accuracy_avg);
						}
					}
				}
			}
...
sub make_test_classifier_or ($$$$$$$$$$$$)
{
	my ($data_base, $support, $confidence, $min_num_rules, $max_num_rank_rules, $ometric, $alpha, $beta, $avg_time, $avg_patterns, $avg_rules, $accuracy) = @_;
	...
	for ($fold = 0; $fold < $Common::NumFolds; $fold++)
	{
		...
		run_classifier_or ($data_base, $fold, $support, $confidence, $min_num_rules, $max_num_rank_rules, $ometric, $alpha, $beta, $log_file, \$acc, \$pat, \$rul, \$tim);
	...
	save_output_file ($data_base, $support, $confidence, $min_num_rules, $max_num_rank_rules, $ometric, $alpha, $beta, $$avg_time, $$avg_patterns, $$avg_rules, $$accuracy);
...
sub run_classifier_or ($$$$$$$$$$$$$$)
{
	my ($data_base, $fold, $support, $confidence, $min_num_rules, $max_num_rank_rules, $ometric, $alpha, $beta, $log_file, $accuracy, $avg_patterns, $avg_rules, $avg_time) = @_;
...
sub save_output_file ($$$$$$$$$$$$)
{
	...
	open OUTPUT, ">$out_file";
	print OUTPUT "support [$support], confidence [$confidence], min_num_rules [$min_num_rules], max_num_rank_rules [$max_num_rank_rules], ometric [$ometric], alpha [$alpha], beta [$beta], avg_time [$avg_time], avg_patterns [$avg_patterns], avg_rules [$avg_rules], accuracy [$accuracy]\n";
	close OUTPUT;
