...
Nesta seção apresentaremos alguns casos de teste extraídos do \textit{log} de execução das abordagens LAC e OLAC.

\subsubsection{Base \textit{breast.ac}}

Serão apresentados, nesta seção, os detalhes do processo de classificação de uma instância de teste em que o LAC obteve sucesso e o OLAC não. A configuração selecionada possui 69 instâncias de teste, das quais o OLAC classificou quatro de maneira errada, e o LAC, apenas duas. Para uma das instâncias com as quais o OLAC não obteve sucesso, o LAC encontrou dez padrões freqüentes, com suporte maior ou igual a 0.001 (tabela \ref{tab:table_lac_x_olac_breast.ac.lac_patterns}), com os quais gerou vinte regras de associação com confiança maior ou igual a 0.01 (tabela \ref{tab:table_lac_x_olac_breast.ac.lac_rules}).

\input{tables/table_lac_x_olac_breast.ac.lac_patterns.tex}
\input{tables/table_lac_x_olac_breast.ac.lac_rules.tex}

TODO: Comentar sobre as regras, a média dos valores de convicção, a classificação correta do LAC. Apresentar as tabelas para o OLAC, comentar sobre as regras, mencionar a regra incorreta que possui convicção muito alta, discutir. Comentar sobre a segunda instância de testes, não é necesário apresentar as tabelas novamente.
