...
\par
Abaixo, fornecemos a definição de algumas das mais utilizadas. Utilizamos o termo $Z$ para designar uma regra associativa, que também pode ser descrita como $X \Rightarrow Y$. O termo $\mathcal{D}$ foi utilizado para designar a base de treinamento. Chamamos de $P(Z)$ a probabilidade de que uma transação qualquer de $\mathcal{D}$ seja coberta por $Z$: \[P(Z) = \frac{frequencia(Z)}{|\mathcal{D}|},\] onde $frequencia(Z)$ é o número de transações em $\mathcal{D}$ cobertas por $Z$.
