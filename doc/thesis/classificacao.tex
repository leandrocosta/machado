...
Note que $|\mathcal{D}| = frequencia(\left\{\right\},\mathcal{D})$. O suporte de um \textit{itemset} $X$ em $\mathcal{D}$ é a probabilidade de $X$ ocorrer em uma transação $T \in \mathcal{D}$: \[suporte(X,\mathcal{D}) := P(X) = \frac{frequencia(X,\mathcal{D})}{|\mathcal{D}|}.\]
