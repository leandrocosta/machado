...
\par
Em \cite{veloso06multi}, os autores utilizam a abordagem \textit{lazy} de classificação associativa direcionado para classificação de documentos. O artigo apresenta um algoritmo de classificação capaz de analisar tanto o conteúdo dos documentos quanto a presença de \textit{links} de saída e de entrada para outros documentos. Os resultados obtidos demonstram que as inovações introduzidas pelos autores produzem mais efetivos e de maior desempenho que abordagens do estado da arte nas coleções utilizadas.
\par
