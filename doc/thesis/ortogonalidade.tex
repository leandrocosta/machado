...
\par
Entretanto, a simples informação de que um determinado conjunto de padrões é ortogonal ou não é insuficiente para que um possível conjunto-solução seja avaliado. Pra tanto, preciso desenvolver uma métrica dada por uma função $\mathcal{F} \rightarrow [\left[0, 1\right]$ que forneça a medida de ortogonalidade do conjunto.
\par
...
\par
Seja $\mathcal{I}$ um conjunto de itens, $\mathcal{D}$ uma base de dados de transações em $\mathcal{I}$, $\mathcal{F}$ o conjunto de padrões freqüentes em $\mathcal{D}$, e $\mathcal{F^{'}}$ um sub-conjunto de $\mathcal{F}$ ($\mathcal{F^{'}} \subseteq \mathcal{F}$). Chamamos de $\mathcal{I^{'}} \subseteq \mathcal{I}$ o sub-conjunto itens que aparecem em, pelo menos, um dos padrões de $\mathcal{F^{'}}$. Para cada item $i \subseteq \mathcal{I^{'}}$ é dado um peso: \[w_i = \frac{|\mathcal{F^{'}}|-k}{|\mathcal{F^{'}}|-1},\] onde $k$ é o número de padrões de $\mathcal{F^{'}}$ que contém o item $i$.
\par
...
\par
Finalmente, a ortogonalidade baseada na estrutura dos padrões do conjunto é dada por: \[O_e = \frac{\sum_{i \subseteq \mathcal{I^{'}}} w_i}{|\mathcal{I^{'}}|}.\]
\par
...
\par
Seja $\mathcal{I}$ um conjunto de itens, $\mathcal{D}$ uma base de dados de transações em $\mathcal{I}$, $\mathcal{F}$ o conjunto de padrões freqüentes em $\mathcal{D}$, e $\mathcal{F^{'}}$ um sub-conjunto de $\mathcal{F}$ ($\mathcal{F^{'}} \subseteq \mathcal{F}$). Chamamos de $\mathcal{D^{'}} \subseteq \mathcal{D}$ o sub-conjunto transações cobertas por, pelo menos, um dos padrões de $\mathcal{F^{'}}$. Para cada transação $t \subseteq \mathcal{D^{'}}$ é dada um peso: \[w_t = \frac{|\mathcal{F^{'}}| - k}{|\mathcal{F^{'}}| - 1},\] onde $k$ é o número de padrões de $\mathcal{F^{'}}$ que cobrem a transação $t$.
\par
...
\par
A ortogonalidade baseada em cobertura de transações do conjunto é dada por: \[O_c = \frac{\sum_{t \subseteq \mathcal{D^{'}}} w_t}{|\mathcal{D^{'}}|}.\]
\par
TODO: inserir um exemplo aqui.
...
\par
Seja $\mathcal{I}$ um conjunto de itens, $\mathcal{D}$ uma base de dados de transações em $\mathcal{I}$, $\mathcal{F}$ o conjunto de padrões freqüentes em $\mathcal{D}$, e $\mathcal{F^{'}}$ um sub-conjunto de $\mathcal{F}$ ($\mathcal{F^{'}} \subseteq \mathcal{F}$). Chamamos de $\mathcal{D^{'}} \subseteq \mathcal{D}$ o sub-conjunto transações cobertas por, pelo menos, um dos padrões de $\mathcal{F^{'}}$. Seja $\mathcal{C}$ um conjunto de classes associadas às transações de $\mathcal{D}$. Chamamos de $\mathcal{C^{'}} \subseteq \mathcal{C}$ o sub-conjunto de classes associadas às transações de $\mathcal{D^{'}}$. Para cada classe $c \subseteq \mathcal{C^{'}}$ é dada um peso: \[w_c = \frac{|\mathcal{F^{'}}| - k}{|\mathcal{F^{'}}| - 1},\] onde $k$ é o número de padrões de $\mathcal{F^{'}}$ que cobrem uma quantidade de transações $t \subseteq \mathcal{D^{'}}$ maior que $90\%$ da média esperada \footnote{Dado um conjunto de transações $\mathcal{D}_c^{'}$ onde todas as transações possuem a classe $c$ e um conjunto de padrões $\mathcal{F^{'}}$ em $\mathcal{D}_c^{'}$, a cobertura esperada de cada padrão para a classe $c$ é $\frac{|\mathcal{D}_c^{'}|}{|\mathcal{F^{'}}|}$.}.
\par
...
\par
Por fim, a ortogonalidade baseada em cobertura de classes é dada por: \[\frac{\sum_{c \subseteq \mathcal{C^{'}}} w_c}{|\mathcal{C^{'}}|}.\]
\par
