...
\par
Seja $Y$ um conjunto de itens. Um conjunto $X = \left\{i_1, \cdots, i_k\right\} \subseteq Y$ é chamado de \textit{itemset}, ou \textit{k-itemset} se ele contém $k$ itens.
\par
Uma transação sobre $Y$ é uma tupla $T = \left(tid, I\right)$ onde $tid$ é o identificador da transação e $I$ é um \textit{itemset}. Dizemos que uma transação $T = \left(tid, I\right)$ suporta um \textit{itemset} $X \subseteq Y$, se $X \subseteq I$.
\par
Uma base de dados de transações $D$ sobre $Y$ é um conjunto de transações sobre $Y$. Omitiremos $Y$ sempre que a sua dedução for trivial no contexto.
\par
A cobertura de um \textit{itemset} $X$ em $D$ consiste no conjunto de identificadores de transações das transações em $D$ que suportam $X$: \[cobertura(x,D):=\left\{tid|(tid,I) \in D,X \subseteq I \right\}.\]
O suporte de um \textit{itemset} $X$ em $D$ é o número de transações cobertas por $X$ em $D$: \[suporte(X,D):=|cobertura(X,D)|.\]
A freqüência de um \textit{itemset} $X$ em $D$ é a probabilidade de $X$ ocorrer em uma transação $T \in D$: \[frequencia(X,D):=P(X)=\frac{suporte(X,D)}{|D|}.\]
Note que $|D| = suporte({},D)$.
\par
Um \textit{itemset} é freqüente se o seu suporte é maior ou igual a um dado valor absoluto mínimo $\sigma_{abs}$, com $0 \leq \sigma_{abs} \leq |D|$. Quando se considera freqüências de \textit{itemsets} ao invés de suportes, é utilizado um valor relativo $\sigma_{rel}$, com $0 \leq \sigma_{rel} \leq 1$. Obviamente, $\sigma_{abs} = \left\lceil \sigma_{rel} \cdot |D|\right\rceil$.

\begin{definition}
Seja $D$ uma base de dados de transações sobre um conjunto de itens $Y$, e $\sigma$ um valor mínimo de suporte. A coleção de \textit{itemsets} freqüentes em $D$ em relação a $\sigma$ é dado por: \[F(D,\sigma):=\left\{X \subseteq Y | suporte (X,D) \geq \sigma \right\}.\]
\end{definition}

\begin{problem}
Dado um conjunto de itens $Y$, uma base de dados de transações $D$ sobre $Y$, e um suporte mínimo $\sigma$, encontre $F(D,\sigma)$.
\end{problem}

Na prática, não estamos interessados apenas no conjunto de \textit{itemsets} $F$, mas também nos respectivos suportes desses \textit{itemsets}.
...
\par
Uma regra de associação é uma aplicação da forma $X \rightarrow I_j$, onde $X$ é um conjunto de itens em $I$, e $I_j$ é um único item em $I$ que não está presente em $X$. A regra $X \rightarrow I_j$ é satisfeita no conjunto de transações $T$ com confiança $0 \leq c \leq 1$ se, e somente se, pelo menos $c$\% das transações em $T$ que satisfazem $X$ também satisfizerem $I_j$ \citep{agrawal93mining}.
\par
O suporte de uma regra $X \rightarrow Y$ em $D$ é o suporte de $X \cup Y$ em $D$, e a freqüência da regra é a freqüência de $X \cup Y$. Dizemos que uma regra de associação é freqüente se o seu suporte excede um determinado valor mínimo. $\sigma_{abs} (\sigma({rel})$.
\par
A confiança de uma regra de associação $X \rightarrow Y$ em $D$ é a probabilidade condicional de encontrar $Y$ numa transação, dado que esta contém $X$: \[confianca(X \rightarrow Y,D):=P(Y|X) = \frac{suporte(X \cup Y,D)}{suporte(X,D)}.\]

Dizemos que a regra é de confiança se $P(X|Y)$ excede um determinado valor mínimo de confiança $\gamma$, com $0 \leq \gamma \leq 1$.

\begin{definition}
Seja $D$ uma base de dados de transações sobre um conjunto de itens $Y$, $\sigma$ um valor mínimo para suporte e $\gamma$ um valor mínimo para confiança, o conjunto de regras de associação freqüentes e de confiança considerando $\sigma$ e $\gamma$ é dado por: \[R(D,\sigma \gamma):=\left\{X \rightarrow Y|X,Y \subseteq Y, X \cap Y = \left\{ \right\}, X \cup Y \in F(D,\sigma),confianca(X \rightarrow Y,D) \geq \gamma\right\}.\]
\end{definition}

\begin{problem}
Dado um conjunto de itens $I$, uma base de dados de transações $D$ sobre $Y$, um valor mínimo para suporte $\sigma$ e um valor mínimo para confiança $\gamma$, encontre $R(D,\sigma,\gamma)$.
\end{problem}
...
\par
A ortogonalidade, particularmente, tem sido explorada em áreas como visualização \citep{cui07}, reconhecimento facial \citep{nagao98}, taxonomia \citep{smith00}, dentre outras.
\par
