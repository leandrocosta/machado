...
my $OUTPUT_DIR = "output/classifier_c";
...
format OUTPUT =
support: @<<<<, confidence: @<<<<, min_num_rules: @<<<, max_num_rank_rules: @<<<, min_rule_len: @<, max_rule_len: @<<<<<<, avg_patterns: @######.######, avg_rules: @######.######, accuracy: @#####.###
         $support,         $confidence,         $min_num_rules,           $max_num_rank_rules, $min_rule_len,   $max_rule_len,           $avg_patterns,             $avg_rules,               $accuracy
.
				...
				for ($l = 0; $l < scalar @max_num_rank_rules; $l++)
				{
					...
					for ($m = 0; $m < scalar @min_rule_len; $m++)
					{
						...
						for ($a = 0; $a < scalar @max_rule_len; $a++)
						{
							...
							my $out_file = "$OUTPUT_DIR/$data_base/s".$support."_c".$confidence."_n".$min_num_rules."_l".$max_num_rank_rules."_m".$min_rule_len."_a".$max_rule_len.".out";
							my $log_file = "$OUTPUT_DIR/$data_base/s".$support."_c".$confidence."_n".$min_num_rules."_l".$max_num_rank_rules."_m".$min_rule_len."_a".$max_rule_len.".log";

							run_classifier_c ($data_base, $training_file, $testing_file, $support, $confidence, $min_num_rules, $max_num_rank_rules, $min_rule_len, $max_rule_len, $log_file);

							system "mkdir -p $OUTPUT_DIR/$data_base/";

							open OUTPUT, ">$out_file";
							print OUTPUT "support: $support, confidence: $confidence, min_num_rules: $min_num_rules, max_num_rank_rules: $max_num_rank_rules, min_rule_len: $min_rule_len, avg_patterns: $avg_patterns, avg_rules: $avg_rules, accuracy: $accuracy\n";
							close OUTPUT;
...
sub run_classifier_c ($$$$$$$$$$)
{
	my ($data_base, $training_file, $testing_file, $support, $confidence, $min_num_rules, $max_num_rank_rules, $min_rule_len, $max_rule_len, $log_file) = @_;

	print "$APP_CLASSIFIER -i $training_file -t $testing_file -s $support -c $confidence -n $min_num_rules -l $max_num_rank_rules -m $min_rule_len -a $max_rule_len -r c -d -1 2&>$log_file\n";
	system "$APP_CLASSIFIER -i $training_file -t $testing_file -s $support -c $confidence -n $min_num_rules -l $max_num_rank_rules -m $min_rule_len -a $max_rule_len -r c -d -1 2&>$log_file";
